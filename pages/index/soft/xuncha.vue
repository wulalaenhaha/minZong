<template>
	...
	<view class="soft" @tap="xiaoshi()">
		...
		<!-- 历史记录中的日期选择 -->
		<!-- <view class="Cdate" :class="abc?'show':'hide'">
			<view @click="one(1)" :class="chooseNow==1?'co1':''">{{q1}}</view>
			<view @click="two(2)" :class="chooseNow==2?'co1':''">{{q2}}</view>
			<view @click="three(3)" :class="chooseNow==3?'co1':''">{{q3}}</view>
		</view> -->
				...
				<view class="place_item_left">
					...
					<view class="txt">
						...
						<view class="dizhi">
							...
							<view class="notice" v-show="item.NoCheckNum==0">提醒打卡</view>
						</view>
						<view class="project">
...
<script>
	...
	export default {
		...
		methods:{
			...
			search(ReligName){                           //搜索按钮
				...
				uni.request({
					...
					data:{
						...
						UserID:getApp().globalData.userID,       //未登陆会显示错误
						Year:year,
						Month:month,       //未录入当月数据
						PageNum:0,
						PageSize:16,
			...
			getDetails(id,name,year,month){ 
				console.log(year)
				console.log(month)
				uni.navigateTo({             
					url:'../jindu/inspect/inspect?ID='+ id+ '&ReligName='+name+'&Year='+year+'&Month='+month
				})
			},
